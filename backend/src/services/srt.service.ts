...
- 히브리어 단어 순서를 최대한 유지한 직독직해 번역
- 자연스러운 현대 한국어로 표현하되, 히브리어 구조 반영
- 각 씬마다 반드시 히브리어 핵심 단어 2-3개를 한글 발음과 뜻으로 표기
- 줄바꿈 문자(\\n)를 사용하여 번역과 단어 설명을 구분
- 한국어와 히브리어 자막이 화면에서 동일한 내용을 표시하도록 동기화
- 문장 끝은 평서형으로 작성 (예: "하였다", "되었다" - "하였네", "되었구나" 등 감탄형 금지)

올바른 예시 (직독직해):
"태초에 창조하셨다 하나님이 하늘을 그리고 땅을\\n[베레시트(태초에) · 바라(창조하셨다) · 엘로힘(하나님이)]"
히브리어: "בְּרֵאשִׁית בָּרָא אֱלֹהִים אֵת הַשָּׁמַיִם וְאֵת הָאָרֶץ"

잘못된 예시 (의역 또는 단어 설명 없음):
"하나님이 태초에 천지를 창조하시니라"
...
- 한국어 번역과 정확히 대응하는 히브리어 구절을 배치하세요.
- 히브리어는 단어 순서 그대로 직독직해되어야 합니다.

⚠️ 반드시 다음 JSON 형식으로만 응답 (한국어는 각 요소가 2줄):
{
  "ko": [
    "태초에 창조하셨다 하나님이 하늘을 그리고 땅을\\n[베레시트(태초에) · 바라(창조하셨다) · 엘로힘(하나님이)]",
    "그리고 땅은 있었다 혼돈 그리고 공허\\n[토후(혼돈) · 보후(공허) · 하예타(있었다)]"
  ],
