...
import { listVideoClips, mergeClips, mergeSceneClips, generateSceneClips, burnSubtitlesToClips, addNarrationToClips, produceFinal, resetClipProcessing } from "../controllers/video.controller";

const router = Router();
router.get("/:id/video-clips", listVideoClips);
router.post("/:id/merge-clips", mergeClips);
router.post("/:id/generate-scene-clips/:sceneNo", generateSceneClips);  // 씬 N 클립 N개 한 번에 시작
