...
// ── Word-safe truncation for Hebrew ──

// ── Context-aware truncation for Hebrew ──
function truncateHebrewWordSafe(text: string, maxLength: number): string {
  if (!text || text.length <= maxLength) return text;
  
  // Hebrew grammatical particles that should not be separated from following word
  const particles = ["את", "של", "על", "אל", "מן", "בין", "עם", "כי", "אם"];
  const prefixes = ["ב", "ל", "מ", "ה", "ו", "כ", "ש"]; // Single letter prefixes
  
  // Priority split points (from best to worst):
  // 1. Major punctuation (sof pasuq, etnachta)
  // 2. Conjunctions (vav at word start)
  // 3. After construct chains
  // 4. Regular word boundaries
  
  let truncated = text.slice(0, maxLength);
  
  // Look for major punctuation first (strongest break points)
  const majorPuncIndex = truncated.lastIndexOf("׃"); // Sof pasuq
  if (majorPuncIndex > 20) {
    return truncated.slice(0, majorPuncIndex);
  }
  
  // Look for atnach (mid-verse pause) ֑
  const atnachIndex = truncated.lastIndexOf("֑");
  if (atnachIndex > 20) {
    return truncated.slice(0, atnachIndex + 1); // Include the mark
  }
  
  // Find all word boundaries
  const words = truncated.split(/\s+/);
  
  // Check if we would break after a particle that needs its object
  const lastWord = words[words.length - 1];
  const secondLastWord = words[words.length - 2];
  
  // Don't break after particles that need their object
  if (secondLastWord && particles.includes(secondLastWord)) {
    // Remove last two words to keep particle with its object
    words.pop();
    words.pop();
    return words.join(" ");
  }
  
  // Check for construct chain (סמיכות) - don't break in middle
  // If last word starts with a prefix and is short, likely part of construct
  if (lastWord && lastWord.length < 4 && /^[בלמהוכש]/.test(lastWord)) {
    words.pop();
    return words.join(" ");
  }
  
  // Look for natural phrase boundaries (words starting with vav)
  for (let i = words.length - 1; i > words.length / 2; i--) {
    if (words[i] && words[i].startsWith("ו")) {
      // Break before the vav-connected word
      return words.slice(0, i).join(" ");
    }
  }
  
  // Default: break at last complete word
  if (text[maxLength] && text[maxLength] !== " ") {
    // We're in middle of a word, remove it
    words.pop();
    return words.join(" ");
  }
...
export const HelloWorld: React.FC<z.infer<typeof myCompSchema>> = ({
  ...
  const rawHe = subs.length === 0
    ...
    : currentSub !== undefined
      ? (currentSub.heText ?? '')
      : (prevSub?.heText ?? '');
  const displayHe = truncateHebrewWordSafe(cleanHebrew(rawHe), 40);
