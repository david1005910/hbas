...
import { Image, RefreshCw, Check, Zap } from "lucide-react";
        ...
        if (status === "done") {
          ...
          if (imageUrl) {
            setKeyframes((prev) => {
              ...
              if (existing) {
                return prev.map((k) => k.sceneNumber === scene ? { ...k, imageUrl } : k);
              }
              return [...prev, { id: `temp-${scene}`, episodeId, sceneNumber: scene, imageUrl, resolution: "1920x1080", isSelected: false, createdAt: new Date().toISOString() }];
            });
          }
        }
            ...
            <div key={scene} className="border border-gold/20 rounded-xl overflow-hidden bg-ink-light">
              <div className="relative aspect-video bg-ink">
                {latest?.imageUrl ? (
                  ...
                  <div className="flex items-center justify-center h-full text-parchment/20">
                    {state?.status === "generating" ? (
                      <div className="text-center">
                        <RefreshCw className="animate-spin mx-auto mb-2 text-gold" size={24} />
                        <p className="text-xs text-gold">생성 중...</p>
                      </div>
                    ) : (
                      <Image size={32} />
                    )}
                ...
                <div className="flex gap-1.5">
                  {latest && !latest.isSelected && (
                    <button
                      ...
                      className="flex items-center gap-1 text-xs px-2 py-1 bg-gold/10 hover:bg-gold/20 text-gold border border-gold/30 rounded transition-colors"
                    >
                      <Zap size={10} />
                      Veo 전송
                    </button>
